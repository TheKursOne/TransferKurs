...
function tampilkan(text) {
  ...
  hasil.innerHTML =
    ...
    '<div style="display:flex;gap:8px;flex-wrap:wrap;">' +
      ...
      (isUrl ? '<a href="' + text + '" target="_blank"><button style="padding:8px 14px;background:#0077ff;border:none;border-radius:6px;cursor:pointer;font-size:12px;color:#fff;width:auto;">Buka Link</button></a>' : '') +
...
window.salinHasil = function() {
  ...
  if (navigator.clipboard && window.isSecureContext) {
    ...
    ta.value = _lastResult; document.body.appendChild(ta);
...
document.getElementById("fileInput").addEventListener("change", function(e) {
  ...
  reader.onload = function() {
    codeReader.decodeFromImage(undefined, reader.result)
      .then(function(result) { tampilkan(result.text); })
      .catch(function() {
        hasil.innerHTML =
